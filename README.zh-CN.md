...
🏗️ **自建（Self-Built）** — 根据目标提出所需角色、员工与技能方案，并由运营者明确审核。

⚙️ **自营（Self-Run）** — 分派工作、驱动交接、恢复持久化运行，并仅在需要人类权限时上报。

🌱 **自成长（Self-Grown）** — 将审核后的结果沉淀为受治理的组织记忆，并衡量后续运行是否真正改善。
...
OpenOPC 当前有意将产品与结果验证范围聚焦在三个方向：
**软件交付、内容生产、基于证据的研究**。这些工作负载既覆盖同一套公司协作闭环，
又足够具体，可以与 Task 模式进行可重复的对照评测。

| 当前重点 | 适合的任务 |
|---|---|
| **💻 软件交付** | 功能、修复、迁移、测试、发布证据与运行恢复 |
| **🎬 内容生产** | 脚本、分镜、营销包与可发布的多格式资产 |
| **🔎 研究** | 有引用的简报、市场图谱、技术比较与决策材料 |

对于由单个执行 Agent 即可负责的直接、边界清晰的工作，使用 **Task 模式**。
当工作确实受益于拆解、并行角色、独立评审与最终集成时，使用 **Company 模式**。

金融、销售、电商、教育等组织模板仍是扩展方向。可用的模板或演示不等于该领域的
Company 模式已经具备生产就绪证据。产品声明必须继续通过仓库内版本化的
Task-versus-Company 基准与晋级档案。

当前产品契约、北极星结果与明确的非目标，见
...
OpenOPC 围绕复杂的真实任务组建一家 AI 公司 — 通过三个紧密耦合的机制：
**自建**负责组织配员，**自营**负责执行工作，**自成长**负责从审核后的结果中学习。
自主性受证据、预算、权限与人类授权约束，而不是以系统尽量少求助为目标。
...
- 🎯 提出配员方案 — 招聘 Agent 比较现有员工、新人、经验、可靠性与成本，再由运营者确认运行。
...
执行产生原始经验；经过审核的执行才会转化为持久改进。候选记忆、技能与策略必须先
通过离线、影子和金丝雀证据门槛，晋级后还要与匹配的对照运行比较实际效果。
